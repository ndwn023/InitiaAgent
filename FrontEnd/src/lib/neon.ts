...
export async function ensureDatabase() {
  ...
  if (!initPromise) {
    initPromise = (async () => {
      ...
      await sql`ALTER TABLE agents ADD COLUMN IF NOT EXISTS roi DOUBLE PRECISION;`;
      await sql`ALTER TABLE agents ADD COLUMN IF NOT EXISTS win_rate DOUBLE PRECISION;`;
      await sql`ALTER TABLE agents ADD COLUMN IF NOT EXISTS total_trades INTEGER;`;
      await sql`ALTER TABLE agents ADD COLUMN IF NOT EXISTS is_demo BOOLEAN NOT NULL DEFAULT FALSE;`;
    })();
  }
