...
export async function getAllAgents(): Promise<StoredAgent[]> {
  ...
  const rows = await sql`SELECT * FROM agents ORDER BY deployed_at DESC, created_at DESC`;
...
export async function addAgent(agent: StoredAgent): Promise<StoredAgent> {
  ...
  await sql`
    INSERT INTO agents (
      ...
      ${agent.status}, ${agent.deployedAt}, ${agent.txHash}, ${agent.contractAddress},
      ${agent.initialCapital}, ${agent.creatorAddress ?? null}, ${agent.interval ?? null},
...
export async function updateAgent(id: string, patch: Partial<StoredAgent>): Promise<StoredAgent | null> {
  ...
  const rows = await sql`
    UPDATE agents SET
      name = ${next.name}, strategy = ${next.strategy}, target = ${next.target ?? null},
      pool = ${next.pool ?? null}, protocol = ${next.protocol ?? null}, vault = ${next.vault ?? null},
      status = ${next.status}, deployed_at = ${next.deployedAt}, tx_hash = ${next.txHash},
      contract_address = ${next.contractAddress}, initial_capital = ${next.initialCapital},
      creator_address = ${next.creatorAddress ?? null}, interval = ${next.interval ?? null},
      is_subscription = ${next.isSubscription ?? false}, agent_closed = ${next.agentClosed ?? false},
      take_profit_pct = ${next.takeProfitPct ?? null}, stop_loss_pct = ${next.stopLossPct ?? null},
      min_confidence = ${next.minConfidence ?? null}, trade_size_pct = ${next.tradeSizePct ?? null},
      on_chain_agent_id = ${next.onChainAgentId ?? null}, updated_at = NOW()
    WHERE id = ${id}
    RETURNING *
  `;
