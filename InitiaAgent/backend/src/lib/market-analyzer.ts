...
function calculateIndicators(price: PriceData): Indicators {
  ...
  if (price.volume24h && price.marketCap) {
    ...
    volumeScore = isHighVol ? (change24h >= 0 ? 1 : -1) : (change24h >= 0 ? 0.3 : -0.3);
  }
  ...
  const compositeScore =
    emaNorm * 0.30 + macdNorm * 0.20 + rsiDev * 0.20 +
    momNorm * 0.20 + volNorm * 0.10 - volPenalty;

  return { emaCrossover, macdSignal, trendStrength, rsi, momentum24h: change24h, bollingerPct, volatilityPct, isSqueeze, volumeScore, compositeScore };
}
...
function generateReasoning(signal: "BUY" | "SELL" | "HOLD", token: string, price: PriceData, ind: Indicators, strategy: string, marketContext: string): string {
  ...
  const rsiNote = ind.rsi > 70 ? " RSI overbought — profit-taking zone (Wilder)." : ind.rsi < 30 ? " RSI oversold — accumulation zone (Wilder)." : "";
  const macdNote = ind.macdSignal > 0.5 ? " MACD histogram positive — bullish crossover (Appel)." : ind.macdSignal < -0.5 ? " MACD histogram negative — bearish crossover (Appel)." : "";
  const volNote = ind.volumeScore > 0.5 ? " High volume confirms upside (Wyckoff demand)." : ind.volumeScore < -0.5 ? " High volume confirms downside (Wyckoff supply)." : ind.volumeScore !== 0 ? " Below-average volume — weak confirmation." : "";
  const bbNote = ind.bollingerPct >= 0.85 ? " At upper Bollinger band — resistance zone." : ind.bollingerPct <= 0.15 ? " At lower Bollinger band — support zone." : ind.isSqueeze ? " Bollinger squeeze — breakout incoming." : "";
  const ctx = marketContext ? ` ${marketContext}` : "";

  return `${base}${rsiNote}${macdNote}${bbNote}${volNote}${ctx}`.trim();
}

function generateSuggestedAction(signal: "BUY" | "SELL" | "HOLD", token: string, price: PriceData, ind: Indicators, strategy: string): string {
  const p = price.price;
  ...
  if (signal === "BUY") {
    if (strategy.toUpperCase().includes("DCA")) return `DCA into ${token} at $${p}. Scale in gradually — next target $${target}. Don't deploy all capital at once.`;
    if (strategy.toUpperCase().includes("LP")) return `Add liquidity near $${p}. Rebalance threshold: >${(ind.volatilityPct * 2).toFixed(1)}% price drift.`;
    return `Enter ${token} at $${p}. Target $${target}, stop $${stop}. R/R ≥ 1.5 (Livermore principle).`;
  }
  if (signal === "SELL") return `Reduce ${token} at $${p}. Support at $${stop}. Protect capital first.`;
  if (ind.isSqueeze) return `Bollinger squeeze on ${token}. Await volume breakout above $${target} or below $${stop} before committing.`;
...
export function analyzeMarketRules(
  strategy: string,
  params: { targetToken?: string; pool?: string; protocol?: string; vault?: string; capital?: number; interval?: string },
  ...
  if (!tokenData) {
    return {
      signal: "HOLD", token: params.targetToken || "INIT", confidence: 35,
      reasoning: "Insufficient market data. Defaulting to HOLD — capital preservation first (Ray Dalio).",
      suggestedAction: "Wait for market data to refresh before taking action.", riskLevel: "Medium",
    };
  }
  ...
  return {
    signal, token: tokenName, confidence,
