...
router.get("/strategy-skills", (_req: Request, res: Response) => {
  res.json({
    strategies: {
      DCA:   { skills: ["Dip Entry Detector", "Accumulation Engine", "Cost Basis Tracker", "DCA Interval Optimizer"], indicators: ["RSI < 35", "EMA 20/50 crossover", "Volume spike +2σ", "24h momentum score"] },
      LP:    { skills: ["IL Protection Guard", "Fee APR Optimizer", "Pool Rebalance Trigger", "Depth Scanner"], indicators: ["Pool ratio deviation >5%", "Volume/TVL ratio", "Fee 24h APR", "Price range delta"] },
      YIELD: { skills: ["APY Scanner", "Protocol Rotation Engine", "Harvest Timer", "Compound Frequency Optimizer"], indicators: ["APY delta 7d", "Emissions decay rate", "Utilization ratio", "Reward token trend"] },
      VIP:   { skills: ["Tier Retention Monitor", "esINIT Compounder", "Epoch Timing Tracker", "Volume Threshold Manager"], indicators: ["esINIT APR", "VIP tier threshold", "Epoch progress %", "Loyalty multiplier"] },
    },
    availableModels: {
      anthropic: ANTHROPIC_MODELS,
      gemini: GEMINI_MODELS,
      claudeCli: process.env.ANTHROPIC_API_KEY ? false : true,
    },
    apiKeys: {
