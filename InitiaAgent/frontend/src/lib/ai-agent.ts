...
// ─── Model cascade ────────────────────────────────────────────────────────────
// Anthropic first (best reasoning), Gemini as fallback

interface ModelEntry { id: string; provider: "anthropic" | "gemini" }
...
async function callModel(
  ...
  if (entry.provider === "anthropic") {
    ...
    if (block.type !== "text" || !block.text) throw new Error("Empty Anthropic response");
...
Respond ONLY with a valid JSON object in this exact format:
{
  ...
  // Try each model in cascade: Anthropic → Gemini
  for (const entry of MODELS) {
    try {
      const text = await callModel(entry, TRADING_SYSTEM_PROMPT, prompt, { temperature: 0.4, maxTokens: 800, jsonMode: entry.provider === "gemini" });
      if (text) {
        console.log(`AI analysis success with model: ${entry.id} (${entry.provider})`);
        return parseAnalysis(text, params.targetToken || "INIT");
      }
    } catch (error: any) {
      console.warn(`Model ${entry.id} failed:`, error?.message?.substring(0, 100) || error);
      continue;
    }
  }

  // All models failed — simulation fallback
  console.error("All AI models failed, using simulation mode");
  ...
  return {
    ...
    confidence: 78,
    reasoning: isLP ? "Volatility within normal range — maintaining liquidity distribution." :
               isDCA ? `Price attractive for long-term accumulation near support.` :
               isYield ? "Scanning protocols for optimized risk-adjusted yield." :
               "Market consolidation detected. Position aligns with risk parameters.",
...
Respond directly to the user's latest message. Use the portfolio context and market data above to give a specific, data-driven answer. Structure your response with bold text for key figures, bullet points for lists, and short paragraphs. Be thorough but concise.`;
  ...
  // Build model list: preferred model first (by id), then cascade
  const preferredId = agentContext?.model;
  let modelList = [...MODELS];
  if (preferredId) {
    const prov = preferredId.startsWith("claude") ? "anthropic" : "gemini";
    modelList = [{ id: preferredId, provider: prov }, ...MODELS.filter(m => m.id !== preferredId)];
  }

  // Try each model in cascade: Anthropic → Gemini
  for (const entry of modelList) {
    try {
      const text = await callModel(entry, CHAT_SYSTEM_PROMPT, prompt, { temperature: 0.6, maxTokens: 2048 });
      if (text) {
        console.log(`Chat success: ${entry.id} (${entry.provider})`);
        return text;
      }
    } catch (error: any) {
      console.warn(`Chat model ${entry.id} failed:`, error?.message?.substring(0, 100) || error);
      continue;
    }
  }
